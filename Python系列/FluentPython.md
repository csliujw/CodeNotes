...
> Python 进阶书，有利于理解 Python 语言，介绍的基本都是高级的 Python 用法. 可以用于全面了解这个语言的能力边界。
...
列表元素重复，不可变对象重复是直接创建一个新的对象，可变对象重复是复制地址，让重复的元素指向同一个对象
...
对于不可变对象，浅拷贝是直接创建一个新的对象，对于可变对象是直接拷贝它的地址值。
...
- <span style="color:green">**赋值运算与深浅拷贝无关**</span>【l1 和 l2 是同一个指向】
...
```python
# ['-'] * 3 是将列表中的元素 '-' 重复三次，这是一个不可变对象，所以每个元素都是独立的
...
<span style="color:blue">Python 默认做浅复制，意思是，在复制不可变对象的时候，是重新创建一个对象，但是在复制可变对象的时候，是复制可变对象的引用。</span>
