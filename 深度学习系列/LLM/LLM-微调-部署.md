...
1️⃣加载预训练模型。如果访问不了 huggingface 就在 Linux 中[配置 huggingface 的镜像](##简介&配置)。

- 配置文件中的 load_in_4bit 表示是否启用 4-bit 量化训练，如果希望模型精度高一些，选择不启用。
...
2️⃣微调前测试模型性能。此处是微调的带思维链的模型。也可以选择微调不带思维链的。
...
- 这里使用 HuggingFace 的 `Conard/fortune-telling` 数据集，是带思维链的数据集 json 格式的。数据可是可以自己定义，重写加载数据集拼凑 prompt 的代码即可。
...
def get_dataset():
    ...
    def formatting_prompts_func(examples):
        # 从数据集中提取问题、思考过程、回答。写法不固定，可以根据数据集的格式进行修改代码。
        instructions = examples["Question"]
        cots         = examples["Complex_CoT"]
        outputs      = examples["Response"]
        texts = []	# 存储格式化后的文本
...
def get_dataset():
    EOS_TOKEN = tokenizer.eos_token # 必须添加结束标记，用于指示文本已经结束
    dataset = load_dataset("Conard/fortune-telling", split = "train[:200]")
    # dataset = load_dataset("json",data_files="/content/all_details.json", split = "train[:200]") 加载本地磁盘的数据
...
- `ollama run qwen3-30a3b keepalive 20m`。模型存活 20 分钟。

> Ollama 可以自定义部署 gguf 文件。以 Qwen3 为例进行说明。

拉取最小的 Qwen3 模型，查看 modelfile 的写法
...
# 模型文件的路径
FROM C:\Users\HP\.ollama\models\blobs\sha256-e9183b5c18a0cf736578c1e3d1cbd4b7e98e3ad3be6176b68c20f156d54a07ac
...
尝试修改 modelfile，让 ollama 加载指定模型（其他地方下载的 gguf 文件），同时关闭 qwen3 的 think 模式。

[how can i disable thinking mode? · Issue #10456 · ollama/ollama](https://github.com/ollama/ollama/issues/10456#issuecomment-2837486127)

```jinja2
FROM C:\Users\HP\qwen:7b.gguf
TEMPLATE """{{- if .Messages }}
{{- if or .System .Tools }}<|im_start|>system
{{- if .System }}
{{ .System }}
...
Ollama 适合小规模场景下的使用。如果是大规模场景下的使用推荐使用 vLLM。做过一个不严谨的测试。DS-32B int4 量化版本的推理速度是 30 token/s，vLLM 32B-QwQ-4bit 量化最高吞吐量是100 token/s。
