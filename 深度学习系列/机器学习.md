...
今天我们学习朴素贝叶斯这个算法。朴素贝叶斯算法 = 朴素+贝叶斯。

我们看看朴素的意思。朴素贝叶斯方法是在贝叶斯算法的基础上进行了相应的简化，即假定给定目标值时属性之间相互条件独立。这个简化方式在一定程度上降低了贝叶斯分类算法的分类效果，但是在实际的应用场景中，极大地简化了贝叶斯方法的复杂性。有点类似于数据结构中的空间换时间、时间换空间。
...
现分别有 A、B 两个容器，在容器 A 里分别有 7 个红球和 3 个白球；在容器 B 里有 1 个红球和 9 个白球。从这两个容器里任意抽出了一个球，且是红球，问这个红球是来自容器 A 的概率是多少?
...
我们是不是通过计算每个事件发生的概率，然后组合这些概率来推测红球来自容器A的概率？

----

朴素贝叶斯算法的核心思想是：对于给出的待分类样本，求解在此样本出现的条件下各个类别出现的概率，哪个最大，就认为此待分类样本属于哪个类别。
