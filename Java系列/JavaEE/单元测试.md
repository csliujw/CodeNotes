...
JavaConfig 类：不能把 mvc 相关的配置导入！！不然会出错！！
...
我特地在和 web 相关的配置（mvc 相关的类，如配置类，前端控制器）上加了 Configuration 注解，排除了那些。
...
<b>PS：</b>Spring 容器中注入的是非 MVC 的相关配置。SpringMVC 的类是在 web 容器中的。

Web 容器用的是 Web 容器中的类。

Spring 容器用的是 Spring 容器中的类。

两种关联的话需要一座桥梁。所以 Spring 和 SpringMVC 需要一些配置进行整合，但是因为是同一家组织的，所以无需三方包进行整合。

进行测试的时候，Spring 归 Spring，MVC 归 MVC；如果双方的 Context 需要相互访问，需要一些设置。

# SpringMVC单元测试

我用的是 maven 库如下: 最好保持一致
...
RootConfig 类, 是用于传统 Spring ApplicationContext 的容器.
