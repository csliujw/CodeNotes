...
public class LRU<K, V> implements Iterable<K> {
	...
	*/
	...
	/*
	* 把数据存入LRU中，如果存在，则把旧值摘除，把新值追加到头部。同时，如果缓存不够，则移除尾部元素
	*/
...
指的是对某个一定不存在的数据【**缓存中没有，数据库中也没有**】进行请求，该请求将会穿透缓存到达数据库。

解决方案：

- 对这些不存在的数据缓存一个空数据，同时需要设置有效期，防止这类无用数据长时间逗留；
- 对这类请求进行过滤。
