...
<span style="color:orange">一个 RedisObject 包含了 8 字节的元数据和一个 8 字节指针</span>，这个指针再进一步指向具体数据类型的实际数据所在，例如指向 String 类型的 SDS 结构所在的内存地址，可以看一下下面的示意图。关于 RedisObject 的具体结构细节，后面详细介绍。
