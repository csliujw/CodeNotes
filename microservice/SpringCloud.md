...
能不使用分布式事务中间件就不要使用。这些中间件的开销太大了，而且不同业务适合的模式又不一样，这又会导致一个系统中，不同的子系统采用的不同的分布式事务中间件模式。而且引入分布式事务中间件会造成系统大规模耦合的情况。是否使用分布式事务中间件是一个取舍问题，我也倾向于尽量不去使用它~。[【IT老齐266】不要在项目中使用分布式事务中间件_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1g84y1s7DH/?spm_id_from=333.1007.top_right_bar_window_history.content.click&vd_source=cb8bc4312b30b416beadaad7244940ac)

<div align="center"><img src="assets/image-20230104171641710.png"></div>
...
<div align="center"><img src="assets/image-20230104172038586.png"></div>

- 同步阻塞：库存扣减不做完不返回结果，库存扣减失败直接返回异常给商城应用。（CP 强一致性）
- 异步调用：订单服务异步调用库存服务，订单服务每 n 秒调用库存服务“出库查询”接口确认订单出库状态。（AP 最终一致性)
    - 出库成功：订单完成，业务结束。
    - 因为技术问题出库失败：重试补偿，不断重试扣减库存直到成功
    - 因为业务问题出库失败(比如库存不足)：撤销订单，短信通知用户、补偿用户等
- TIPS1：尽量避免分布式事务，初期服务粒度可以粗一些！
- TIPS2：对于异步调用，一定要做好资源的校验与锁定，极力避免业务问题产生。
- TIPS3：分布式一致性问题会随着调用层级的增加 (A->B->C->D->E) 难度激增，因此不建议超过 3 层调用层级，如果出现可以考虑服务聚合缩短调用链 (A-> (BC)->(DE))，将 BC 合并成一个服务。
