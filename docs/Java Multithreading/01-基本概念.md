...
先看完的书，再刷的视频，最终笔记以视频中的内容为主线。为了节省篇幅，代码书写可能不太规范。
...
- synchronized 包裹的代码块，在字节码中的体现是 **Monitor** 指令；其中字节码里面会出现两个 **monitorexit**，第二个 **monitorexit** 是为了保证出现异常时 synchronized 也可以释放锁
- synchrozied 使用在方法上，在字节码中的体现是 **ACC_SYNCHRONIZED**，标志表示方法为**同步方法**

监视器是一种同步机制，保证同一时间，只有一个线程可以访问被保护的数据或代码。JVM 同步是基于进入和退出的，而进入和退出是使用管程对象实现的。synchronized 锁的是对象，锁住后会关联一个 **monitor** 对象。（对象会持有 monitor 对象，monitor 对象中会填入持有锁的线程 id）
...
- **②volatile 规则**：volatile变量的写先于读发生，这保证了 volatile 变量的可见性.
- **③锁规则**：解锁（unlock）必然发生在随后的加锁（lock）前.
- **④传递性**：A 先于 B，B 先 于C，那么 A 必然先于 C.
...
并发不一定要依赖多线程（如 `PHP` 中很常见的多进程并发），但是在 Java里面谈论并发，基本上都与线程脱不开关系.
...
方法级别的锁：普通成员方法上加 synchronized，锁的是 this 对象，静态方法上加 synchronized 锁的是字节码对象! **注意：锁的都是对象。**
